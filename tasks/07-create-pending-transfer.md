...
Criar uma transferência entre duas contas existentes, garantindo idempotência por meio de uma chave UUID e persistindo-a inicialmente com status `Pending`.

## Endpoint

```http
POST /api/v1/Transfer
Idempotency-Key: 01900000-0000-0000-0000-000000000004
```
...
- Obter a chave UUID pelo header `Idempotency-Key`.
...
- A chave de idempotência deve ser um UUID válido e diferente de `Guid.Empty`.
- As duas contas devem existir.

## Comportamento idempotente

Antes de criar uma transferência, consultar pelo UUID informado como chave de idempotência:
...
- Chave de idempotência ausente, inválida ou igual a `Guid.Empty` resulta em HTTP 400.
